module Admin
  class PostsController < Admin::ApplicationController
    ...
    def chul_list
      ...
      for i in 1..31
        s = rand(1..98)
        hash = {:date => "2018-02-#{i < 10 ? '0' + i.to_s : i}", :time => "09:#{rand(35..59)}:#{s < 10 ? '0' + s.to_s : s}", :mode => '출근'}
        chul_list.push(hash)
        m = rand(1..59)
        s = rand(1..98)
        hash = {:date => "2018-02-#{i < 10 ? '0' + i.to_s : i}", :time => "19:#{m < 10 ? '0' + m.to_s : m}:#{s < 10 ? '0' + s.to_s : s}", :mode => '퇴근'}
        chul_list.push(hash)
      end
